...
router.get('/check/:userId/:productId', (req, res) => {
  ...
  connection.query('SELECT * FROM fav_list WHERE user_id = ? AND prod_id = ?', [userId, productId], (error, results, fields) => {
    if (error) {
      console.error(error);
    ...
    if (results.length > 0) {
      // Product is already in the fav list for the user
      res.json({
        inFav: true,
        favItemId: results[0].id
      });
    } else {
